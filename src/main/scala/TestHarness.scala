...
class InputGen(idx: Int, prio: Int)(implicit val p: Parameters) extends Module with HasAstroNoCParams {
  ...
  val can_fire = (flits_left === 0.U || (flits_left === 1.U && io.out.fire())) && io.rob_ready

  val packet_remaining = (LFSR(10) % maxFlits.U)
  io.out.valid := LFSR(3) =/= 0.U && flits_left === 0.U && io.rob_ready
...
class NoCTester(inputParams: Seq[ChannelParams], outputParams: Seq[ChannelParams])(implicit val p: Parameters) extends Module with HasAstroNoCParams {
  ...
  val txs_incr = WireInit(VecInit(Seq.fill(nOutputs) { false.B }))
  txs := txs + PopCount(txs_incr)
  io.success := txs >= 100.U

  val tsc = RegInit(0.U((flitPayloadBits-16).W))
  tsc := tsc + 1.U

  val idle_counter = RegInit(0.U(11.W))
  val idle = WireInit(true.B)
  ...
  io.to_noc.map { i =>
    i.flits.zipWithIndex.map { case (f,o) =>
      ...
      igen.io.rob_ready := rob_alloc_avail(idx) && (PopCount(~rob_valids) >= nInputs.U) && tsc >= 10.U
      igen.io.tsc := tsc
      f <> igen.io.out
      when (igen.io.fire) {
